...
public class Person {
    ...
    // Constructors
    public Person() {
        super();    // implicitly included, if the class has no declared parent, this invokes the Object class's constructor
        this.skills = new String[] {"Java", "JavaScript", "HTML", "CSS"};
    }

    public Person(String fn, String ln, String email) {
//        super();    // Can't call both super() and this() in a function - both must come first
        this();     // Constructor chaining must be on first line
        firstName = fn;
        lastName = ln;
        this.email = email;

        // Though not always required, almost always use "this." when referring to object variables
    }

    public Person(String fn, String ln, String email, int age) {
        this(fn, ln, email);    // Constructor chaining
        this.age = age;
    }
    ...
    public String getFirstName() {
        return this.firstName;  // this not required in this case
    }
    ...
    @Override   // technically not required, but good practice to include so that it's obvious that we are overriding an inherited method
