...
//        stringArr[4] = 42; // does not work, because this array only stores String values
        ...
        // Arrays in Java are of a fixed length, once instantiated their size will not change
        // until the reference is reassigned to some other array object.

        // Making a new array that contains the same values as a nother but is of a different size
        ...
        // called: for-each loop, enhanced for loop
...
//        int[][] intArray_2d = new int[4][3];
        ...
        int[] arr = { 32, 65, 232, 0, -1, 42, 93, 93, 4 };
    ...
    public static int findMaxValue(int[] arr) {
        ...
        int currenthighest = arr[0];
        for(int i = 1; i < arr.length ; i++)
            if(currenthighest < arr[i])
    ...
    // must be the last param in the method signature
    // other declared parameters are required.
